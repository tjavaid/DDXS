...
void storeTreeAndTemplatesXS(TTree* TT, TString obsName, TString obsBinDn, TString obsBinUp, TString obsName2, TString obsBinDn2, TString obsBinUp2, TString sfinalState, TString fLocation, TString templateNameTag, TString fOption, TString fitTypeZ4l, bool useRefit);
...
void fiducialXSTemplates(TString processNameTag = "qqZZ", TString processFileName = "ZZTo2e2mu_mZZ95-160.root", TString sfinalState = "4l", TString obsName = "massZ1", TString obsBinDn = "0", TString obsBinUp = "120", TString obsName2 = "massZ2", TString obsBinDn2 = "0", TString obsBinUp2 = "120", TString sqrtsTag = "8TeV", TString baseDirXS = "templatesXS", TString sProcessingType = "DTreeXS", TString fitTypeZ4l = "none", bool useRefit = false){
...
//    templatesXS(processNameTag, processFileName, sqrtsTag, sfinalState, obsName, obsBinDn, obsBinUp, fitTypeZ4l, useRefit);
    templatesXS(processNameTag, processFileName, sqrtsTag, sfinalState, obsName, obsBinDn, obsBinUp, obsName2, obsBinDn2, obsBinUp2,fitTypeZ4l, useRefit);
}
...
//adding extra lines for second observables one bin


    if (obsBinDn==obsBinUp && obsBinDn.Contains("|")) { // if bin boundaries are passed - get the  boundaries in TObjArray and loop
        TObjArray* ta = (TObjArray*) obsBinDn.Tokenize("|");
//below 3 lines are for second obs. with one bin
	TObjArray* ta2 = (TObjArray*) obsBinDn2.Tokenize("|");
	TString obsBinDn2 = ((TObjString*) ta2->At(0))->GetString();
	cout<<"obsBinDn2 is....: "<<obsBinDn2<<endl;
	TString obsBinUp2 = ((TObjString*) ta2->At(0+1))->GetString();
	cout<<"obsBinUp2 is....: "<<obsBinUp2<<endl;
//
        for (int kEntry = 0; kEntry < ta->GetEntries() - 1; kEntry++){
            TString obsBinDn = ((TObjString*) ta->At(kEntry))->GetString();
            TString obsBinUp = ((TObjString*) ta->At(kEntry+1))->GetString();
            cout<<"test1: "<<endl;
//            storeTreeAndTemplatesXS(TT, obsName, obsBinDn, obsBinUp, sfinalState, fLocation, templateNameTag, fOption, fitTypeZ4l, useRefit);
            storeTreeAndTemplatesXS(TT, obsName, obsBinDn, obsBinUp, obsName2, obsBinDn2, obsBinUp2, sfinalState, fLocation, templateNameTag, fOption, fitTypeZ4l, useRefit);
        }
       }
