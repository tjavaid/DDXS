...
def geteffs(channel,List, m4l_bins, m4l_low, m4l_high, obs_reco, obs_gen, obs_bins, recobin, genbin, obs_reco2, obs_gen2, obs_bins2):
...
## for other variable,for double XS   
    obs_reco2_low = obs_bins2[0]
    print "obs_reco2_low is............",obs_reco2_low
    obs_reco2_high = obs_bins2[0+1]
    print "obs_reco2_high is............",obs_reco2_high

    obs_gen2_low = obs_bins2[0]
    obs_gen2_high = obs_bins2[0+1]
...
####################################################
    ...
    for Sample in List:
        ...
        processBin = shortname+'_'+channel+'_'+opt.OBSNAME+'_genbin'+str(genbin)+'_recobin'+str(recobin)+'_'+opt.OBSNAME2+'_'+obs_reco2_low+'_'+obs_reco2_high
        ...
        cutobs_reco = "("+obs_reco+">="+str(obs_reco_low)+" && "+obs_reco+"<"+str(obs_reco_high)+" && "+obs_reco2+">="+str(obs_reco2_low)+" && "+obs_reco2+"<"+str(obs_reco2_high)+")"
	print "cutobs_reco is..............", cutobs_reco
        #cutobs_gen = "("+obs_gen+">="+str(obs_gen_low)+" && "+obs_gen+"<"+str(obs_gen_high)+")"
        cutobs_gen = "("+obs_gen+">="+str(obs_gen_low)+" && "+obs_gen+"<"+str(obs_gen_high)+" && "+obs_gen2+">="+str(obs_gen2_low)+" && "+obs_gen2+"<"+str(obs_gen2_high)+")"


## need to confirm whether we need similar modification for jet observables, or need to add only mass cuts.................
        if (("jet" in opt.OBSNAME) or ("Jet" in opt.OBSNAME)):
            ...
            cutobs_reco_jesup = "("+obs_reco+"_jesup"+">="+str(obs_reco_low)+" && "+obs_reco+"_jesup"+"<"+str(obs_reco_high)+" && "+obs_reco2+"_jesup"+">="+str(obs_reco2_low)+" && "+obs_reco2+"_jesup"+"<"+str(obs_reco2_high)+")"
            #cutobs_reco_jesdn = "("+obs_reco+"_jesdn"+">="+str(obs_reco_low)+" && "+obs_reco+"_jesdn"+"<"+str(obs_reco_high)+")"
            cutobs_reco_jesdn = "("+obs_reco+"_jesdn"+">="+str(obs_reco_low)+" && "+obs_reco+"_jesdn"+"<"+str(obs_reco_high)+" && "+obs_reco2+"_jesdn"+">="+str(obs_reco2_low)+" && "+obs_reco2+"_jesdn"+"<"+str(obs_reco2_high)+")"
        ...
        cutobs_gen_otherfid = "(("+obs_gen+"<"+str(obs_gen_low)+" && "+obs_gen+">="+str(obs_gen_lowest)+" && "+obs_gen2+">="+str(obs_gen2_low)+" && "+obs_gen2+"<"+str(obs_gen2_high)+") || ("+obs_gen+">="+str(obs_gen_high)+" && "+obs_gen+"<="+str(obs_gen_highest)+" && "+obs_gen2+">="+str(obs_gen2_low)+" && "+obs_gen2+"<"+str(obs_gen2_high)+"))"
...
# for double cross section measurement
if (opt.OBSNAME2 == "massZ1"):
    obs_reco2 = "massZ1"
#    obs_gen = "GENmZ1"
    obs_gen2 = "GENmassZ1"
if (opt.OBSNAME2 == "massZ2"):
    obs_reco2 = "massZ2"
#    obs_gen = "GENmZ2"
    obs_gen2 = "GENmassZ2"
...
for chan in chans:
    for recobin in range(len(obs_bins)-1):
        for genbin in range(len(obs_bins)-1):
            ...
            geteffs(chan,List, m4l_bins, m4l_low, m4l_high, obs_reco, obs_gen, obs_bins, recobin, genbin, obs_reco2, obs_gen2, obs_bins2)    # for double cross section  measurement   
...
with open('datacardInputs/inputs_sig_'+opt.OBSNAME+'_vs_'+opt.OBSNAME2+'_'+obs_bins2[0]+'_'+obs_bins2[1]+ext+'.py', 'w') as f:
...
with open('datacardInputs/moreinputs_sig_'+opt.OBSNAME+'_vs_'+opt.OBSNAME2+'_'+obs_bins2[0]+'_'+obs_bins2[1]+ext+'.py', 'w') as f:
